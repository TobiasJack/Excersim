public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support)
        => support switch
        {
                Manager m => "Too Important for a Security Pass",
                Chairman c => "Too Important for a Security Pass",                
                SecurityJunior sj => sj.Title,
                SecurityIntern si => si.Title,
                PoliceLiaison pl => pl.Title,
                Security s => $"{s.Title} Priority Personnel",
                TeamSupport ts => ts.Title
        };             
